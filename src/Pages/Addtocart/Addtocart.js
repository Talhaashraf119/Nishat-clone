...
const Addtocart = () => {
  ...
  const updatestock = async () => {
    ...
    try {
      for (const item of results) {
        ...
        let response = await axios.put(`https://nishat-clone-backend.vercel.app/${item.id}`, {
          stock: newStock
        });
  ...
  const handleCheckout = async () => {
    try {
      await updatestock();  // Update the stock first
     // await makepayment();  // Then proceed to payment
    } catch (error) {
      Swal.fire({
        icon: 'error',
