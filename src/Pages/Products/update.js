 ...
 const navigate= useNavigate()
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        const result = await axios.get(`https://nishat-clone-backend.vercel.app/getdata/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://nishat-clone-backend.vercel.app/updateproduct/${id}`, updatedFormData);
      if (response) {
        Swal.fire({
          icon: 'success',
